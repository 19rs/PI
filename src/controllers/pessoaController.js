...
export async function updatePessoa(req, res) {
    ...
    try {
        stmt = await db.prepare(`UPDATE Pessoas SET nome = ?, cpf = ?, genero = ?, olhos = ?, altura_estimada = ?, peso_estimado = ?, cabelo = ?, caracteristicas_fisicas = ?, vestimentas = ?, residente_em = ?, foto = ?, data_desaparecimento = ?, local_desaparecimento = ?, detalhes_desaparecimento = ?, contato = ? WHERE id = ?`)
        await stmt.bind([ pessoa.nome, pessoa.cpf, pessoa.genero, pessoa.olhos, pessoa.altura_estimada, pessoa.peso_estimado, pessoa.cabelo, pessoa.caracteristicas_fisicas, pessoa.vestimentas, pessoa.residente_em, pessoa.foto, pessoa.data_desaparecimento, pessoa.local_desaparecimento, pessoa.detalhes_desaparecimento, pessoa.contato, pessoa.id ])
        await stmt.run()
